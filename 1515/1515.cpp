...
void transform(char flag,Node& next,Node front)
{
	...
	if(flag == 'A'){
		...
		for(i = 0; i < 2; i++){
			for(j = 0; j < 2; j++){
				int t = 2 + j;
				next.m[i][j] = front.m[i][t];
				next.m[i][t] = front.m[i][j];
			}
		}
	}
	else if(flag == 'B'){
		...
		for(i = 0; i < 2; i++){
			for(j = 0; j < 3; j++){
				next.m[i][j] = front.m[i][j + 1];
			}
			next.m[i][3] = front.m[i][0];
		}
	}
	else{
		next.str = front.str + "C";
		next.m[0][0] = front.m[0][0];
		next.m[0][1] = front.m[0][2];
		next.m[0][2] = front.m[1][2];
		next.m[0][3] = front.m[0][3];
		next.m[1][0] = front.m[1][0];
		next.m[1][1] = front.m[0][1];
		next.m[1][2] = front.m[1][1];
